...
int mdio_register(struct mii_dev *bus)
{
    ...
    if (miiphy_get_dev_by_name(bus->name)) {
        ...
        //        bus->name);
        print("mdio_register: non unique device name\n");
        return -1;
    }
...
int mdio_register_seq(struct mii_dev *bus, int seq)
{
    ...
    bus->name[2] = 'h';
    ret = 3;
    for (int i = 3; i < MDIO_NAME_LEN; ++i) {
        ret = i + 1;
        bus->name[i] = buffer[i - 3];
        if (bus->name[i] == 0) {
            break;
        }
    }
...
{
    print("mdio_list_devices not implemented\n");
    // struct list_head *entry;
...
int miiphy_set_current_dev(const char *devname)
{
    ...
    print("miiphy_set_current_dev: No such device\n");
    return 1;
}
...
struct phy_device *mdio_phydev_for_ethname(const char *ethname)
{
    ...
    print("mdio_phydev_for_ethname: not a known ethernet\n");
    return NULL;
}
...
{
    print("miiphy_listdev not implemented\n");
    // struct list_head *entry;
    // struct mii_dev *dev;
