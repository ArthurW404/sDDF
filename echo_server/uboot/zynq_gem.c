...
volatile struct zynq_gem_regs *eth_regs = (void *)(uintptr_t)0x2000000;
...
static inline int mdio_wait(volatile struct zynq_gem_regs *regs)
{
    u32 timeout = 20000;
...
static u32 phy_setup_op(volatile struct zynq_gem_regs *regs, u32 phy_addr, u32 regnum,
                        u32 op, u16 *data)
{
    u32 mgtcr;
    if (mdio_wait(regs)) {
...
static u32 phyread(volatile struct zynq_gem_regs *regs, u32 phy_addr, u32 regnum, u16 *val)
{
    u32 ret;

    ret = phy_setup_op(regs, phy_addr, regnum,
...
static u32 phywrite(volatile struct zynq_gem_regs *regs, u32 phy_addr, u32 regnum, u16 data)
{
    debug("%s: phy_addr %d, regnum 0x%x, data 0x%x\n", __func__, phy_addr,
          regnum, data);

    return phy_setup_op(regs, phy_addr, regnum,
                        ZYNQ_GEM_PHYMNTNC_OP_W_MASK, &data);
}

int phy_detection(int *phyaddr)
{
    int i;
    u16 phyreg;

    if (*phyaddr != -1) {
        phyread(eth_regs, *phyaddr, PHY_DETECT_REG, &phyreg);
        if ((phyreg != 0xFFFF) &&
    ...
    if (*phyaddr == -1) {
        ...
        for (i = 31; i >= 0; i--) {
            phyread(eth_regs, i, PHY_DETECT_REG, &phyreg);
            if ((phyreg != 0xFFFF) &&
                ((phyreg & PHY_DETECT_MASK) == PHY_DETECT_MASK)) {
                /* Found a valid PHY address */
                *phyaddr = i;
...
//     volatile struct zynq_gem_regs *regs = (struct zynq_gem_regs *)dev->iobase;
...
void zynq_gem_prom_enable(volatile struct zynq_gem_regs *regs)
{
...
void zynq_gem_prom_disable(volatile struct zynq_gem_regs *regs)
{
...
int zynq_gem_start_send(volatile struct zynq_gem_regs *regs, uintptr_t txbase)
{
...
int zynq_gem_recv_enabled(volatile struct zynq_gem_regs *regs)
{
    uint32_t val;
...
void zynq_gem_recv_enable(volatile struct zynq_gem_regs *regs)
{
    /* Enable Receive */
    setbits_le32(&regs->nwctrl, ZYNQ_GEM_NWCTRL_RXEN_MASK);
}

void zynq_gem_halt(volatile struct zynq_gem_regs *regs)
{
                               ...
                               uchar reg, ushort *val)
{
    ...
    ret = phyread(eth_regs, addr, reg, val);
    debug("%s 0x%x, 0x%x, 0x%x\n", __func__, addr, reg, *val);
    return ret;
}

int zynq_gem_miiphy_write(const char *devname, uchar addr,
                                 uchar reg, ushort val)
{
    debug("%s 0x%x, 0x%x, 0x%x\n", __func__, addr, reg, val);
    return phywrite(eth_regs, addr, reg, val);
}
...
//     // sprint(dev->name, "Gem.%lx", base_addr);

//     dev->iobase = base_addr;

//     /* fill enetaddr */
//     volatile struct zynq_gem_regs *regs = (struct zynq_gem_regs *)base_addr;
