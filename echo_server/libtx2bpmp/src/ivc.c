			    ...
			    uint32_t frame_size)
{
	...
	if ((uint64_t)nframes * (uint64_t)frame_size >= 0x100000000) {
		sel4cp_dbg_puts("tegra_ivc: nframes * frame_size overflows\n");
		return -EINVAL;
	}
	...
	if ((qbase1 & (TEGRA_IVC_ALIGN - 1)) ||
		...
		sel4cp_dbg_puts("tegra_ivc: channel start not aligned\n");
		return -EINVAL;
	}

	if (frame_size & (TEGRA_IVC_ALIGN - 1)) {
		sel4cp_dbg_puts("tegra_ivc: frame size not adequately aligned\n");
		return -EINVAL;
	}
	...
	if (ret) {
		sel4cp_dbg_puts("tegra_ivc: queue regions overlap\n");
		return ret;
	}
